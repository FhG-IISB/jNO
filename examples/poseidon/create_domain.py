...
DATA_DIR = os.environ.get("DATA_DIR", "/home/b8cl/projects/DATA")
...
def create_training_domain(SAMPLES):
    ...
    domain = SAMPLES * jno.domain(jno.domain.poseidon(), compute_mesh_connectivity=False)
    x, y, t = domain.variable("interior")
    domain.variable("_t", time[:, None, :])
    domain.variable("_f", _f[:, None, None, :, :, :])
    domain.variable("_u", _u[:, None, None, :, :, :])

    domain.save(f"{OUTPUT_DIR}/domain_{SAMPLES}_inference.pkl")
...
def create_inference_domain():
    ...
    _f2 = np.load(f"{OUTPUT_DIR}/poisson_dataset_sources.npy")[..., np.newaxis]
    _u2 = np.load(f"{OUTPUT_DIR}/poisson_dataset_solutions.npy")[..., np.newaxis]
    ...
    domain = SAMPLES * jno.domain(jno.domain.poseidon(), compute_mesh_connectivity=False)
    domain.variable("t", time)
    domain.variable("_f", _f)
    domain.variable("_u", _u)

    domain.save(f"{OUTPUT_DIR}/inference_domain_{SAMPLES}.pkl")

    print(f"Created domain with {SAMPLES} samples -> _f.shape = {domain.context['_f'].shape} and _u.shape = {domain.context['_u'].shape} and t.shape = {domain.context['t'].shape}")
