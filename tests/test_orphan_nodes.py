...
def test_keep_orphan_nodes_opt_out():
    dom = jno.domain(
        lambda geo: (_mesh_with_orphan(), 3, 0.1), compute_mesh_connectivity=False, keep_orphan_nodes=True
    )
...
def test_clean_mesh_is_untouched():
    ...
    dom = jno.domain(lambda geo: (meshio.Mesh(pts, [("tetra", np.array([[0, 1, 2, 3]]))]), 3, 0.1),
