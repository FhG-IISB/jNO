"""Parameter continuation — ``fem.solve(continuation=jno.solve.continuation(name=values))``.
...
def test_a_linear_sweep_matches_independent_solves():
    ...
    U = fem.solve(continuation=jno.solve.continuation(kap=ks, keep="all"), q=jnp.array([1.0]))
    ...
    U_lu = fem.solve(continuation=jno.solve.continuation(kap=ks, keep="all"), q=jnp.array([1.0]), linear=jno.solve.lu())
    for i in (0, 2, 4):
        ref = fem.solve(continuation=jno.solve.continuation(kap=ks[i : i + 1]), q=jnp.array([1.0]), linear=jno.solve.lu())
    ...
    last = fem.solve(continuation=jno.solve.continuation(kap=ks), q=jnp.array([1.0]))
...
def test_two_parameters_march_together_and_fixed_kwargs_hold():
    ...
    fem = _poisson_kap()
    U = fem.solve(continuation=jno.solve.continuation(kap=jnp.array([1.0, 2.0]), q=jnp.array([1.0, 2.0]), keep="all"))
...
def test_homotopy_reaches_a_load_the_cold_solve_cannot():
    ...
    with pytest.raises(RuntimeError, match=r"step 1/1 at load="):
        fem.solve(continuation=jno.solve.continuation(load=jnp.array([0.1])))

    ramped = fem.solve(continuation=jno.solve.continuation(load=jnp.linspace(0.025, 0.1, 4)))
    assert bool(jnp.isfinite(ramped).all())

    ref = fem.solve(
        continuation=jno.solve.continuation(load=jnp.array([0.1])),
...
def test_a_complex_sweep_returns_the_complex_family():
    ...
    U = fem.solve(continuation=jno.solve.continuation(sig=sigs, keep="all"))
...
def test_a_failing_step_names_the_value_and_index():
    ...
    with pytest.raises(RuntimeError, match=r"step 3/4 at kap=.*non-finite|step 3/4 at kap="):
        fem.solve(
            continuation=jno.solve.continuation(kap=jnp.array([1.0, 0.5, 0.0, -0.5]), keep="all"),
            q=jnp.array([1.0]),
            linear=jno.solve.lu(),
        )
...
def test_validation_fails_loud():
    ...
    with pytest.raises(TypeError, match="unknown runtime parameter"):
        fem.solve(continuation=jno.solve.continuation(nope=jnp.array([1.0])))
    with pytest.raises(ValueError, match="share one length"):
        fem.solve(continuation=jno.solve.continuation(kap=jnp.array([1.0, 2.0]), q=jnp.array([1.0])))
    with pytest.raises(ValueError, match="either marched or held"):
        fem.solve(continuation=jno.solve.continuation(kap=jnp.array([1.0])), kap=jnp.array([2.0]))
    ...
    with pytest.raises(NotImplementedError, match="steady"):
        fem_t.solve(continuation=jno.solve.continuation(kap=jnp.array([1.0, 2.0])))
...
def test_the_slots_compose_with_the_sweep():
    ...
    a = fem.solve(continuation=jno.solve.continuation(kap=ks, keep="all"), q=jnp.array([1.0]))
    b = fem.solve(
        continuation=jno.solve.continuation(kap=ks, keep="all"),
        q=jnp.array([1.0]),
