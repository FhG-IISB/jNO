...
# jaxamg wraps NVIDIA AmgX (GPU-only) — a real solve also needs a CUDA GPU backend, not just the package.
_JAXAMG_RUNNABLE = _HAS_JAXAMG and jax.default_backend() == "gpu"
...
@pytest.mark.slow
@pytest.mark.skipif(not _JAXAMG_RUNNABLE, reason="requires jaxamg + AmgX + a CUDA GPU")
