...
class TestNNMLP:
    def test_returns_flax_module(self):
        key = jax.random.PRNGKey(0)
        m = nn.wrap(foundax.mlp(3, output_dim=1, hidden_dims=16, num_layers=2, key=key))
    ...
    def test_different_configs(self):
        k1, k2 = jax.random.split(jax.random.PRNGKey(0))
        m1 = nn.wrap(foundax.mlp(3, output_dim=1, hidden_dims=32, num_layers=3, key=k1))
        m2 = nn.wrap(foundax.mlp(3, output_dim=2, hidden_dims=16, num_layers=1, key=k2))
        assert m1.layer_id != m2.layer_id

    def test_forward_shape(self):
        """Verify forward pass output shape."""
        key = jax.random.PRNGKey(0)
        m = nn.wrap(foundax.mlp(3, output_dim=4, hidden_dims=16, num_layers=2, key=key))
        x = jnp.ones((5, 3))
        y = m.module(x)
        assert y.shape == (5, 4)
    ...
    def test_default_seed_allows_mlp_without_key(self):
        set_default_rng_seed(123)
        try:
            m = nn.wrap(foundax.mlp(3, output_dim=1, hidden_dims=8, num_layers=2))
            assert isinstance(m, Model)
            y = m.module(jnp.ones((2, 3)))
            assert y.shape == (2, 1)
        finally:
...
class TestFNO1D:
    def test_returns_flax_module(self):
        key = jax.random.PRNGKey(0)
        m = nn.wrap(foundax.fno1d(1, hidden_channels=8, n_modes=4, d_vars=1, key=key))
        assert isinstance(m, Model)
    ...
    def test_forward_shape(self):
        key = jax.random.PRNGKey(0)
        m = nn.wrap(foundax.fno1d(1, hidden_channels=8, n_modes=4, d_vars=1, n_layers=2, key=key))
