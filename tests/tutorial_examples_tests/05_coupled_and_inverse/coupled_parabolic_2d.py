...
u_net = jno.np.nn.mlp(in_features=3, hidden_dims=48, num_layers=4, key=jax.random.PRNGKey(24))
v_net = jno.np.nn.mlp(in_features=3, hidden_dims=48, num_layers=4, key=jax.random.PRNGKey(25))
for net in [u_net, v_net]:
    net.optimizer(optax.adam(1), lr=lrs.warmup_cosine(10, 1, 1e-3, 1e-5))

u = u_net(x, y, t) * x * (1 - x) * y * (1 - y)
v = v_net(x, y, t) * x * (1 - x) * y * (1 - y)
u0 = u_net(x0, y0, t0) * x0 * (1 - x0) * y0 * (1 - y0)
v0 = v_net(x0, y0, t0) * x0 * (1 - x0) * y0 * (1 - y0)
