...
# ── Domains ───────────────────────────────────────────────────────────────────
# Dense interior collocation points — used for PDE residual and interaction
x_col = np.linspace(0, 1, 51)[1:-1].reshape(-1, 1)   # 49 interior points
...
# Combine both sets into one domain with two tags
domain = jno.domain.from_array({"interior": x_col, "obs": x_sen})
x,   _ = domain.variable("interior")   # collocation variable
x_s, _ = domain.variable("obs")        # sensor variable

# ── Networks ──────────────────────────────────────────────────────────────────
key = jax.random.PRNGKey(0)
k1, k2 = jax.random.split(key)

u_phy_net = jno.nn.wrap(
    foundax.mlp(in_features=1, output_dim=1, hidden_dims=32, num_layers=3, key=k1)
)
u_syn_net = jno.nn.wrap(
    foundax.mlp(in_features=1, output_dim=1, hidden_dims=32, num_layers=3, key=k2)
)
