...
This script trains a PINN for the Poisson problem above, then verifies the
divergence theorem at evaluation time by comparing the volume integral
∫_Ω Δu dΩ with the boundary flux ∫_∂Ω ∇u · n dS. Outward-normal Variables
are obtained by passing ``normals=True`` to ``domain.variable``.
"""
...
n = jno.np.vector(nx, ny)  # VectorView of outward normal
boundary_flux = grad_u_bnd.dot(n).integrate()
