# --8<-- [start:code]
"""Elastodynamics + modal analysis: a ringing cantilever (vector, 2nd-order time) via ``jno.fem``.

The transient sibling of the static cantilever (linear-elasticity tutorial). A plane-stress elastic
beam clamped at the root obeys Newton's second law for a continuum,
...
Precision note: the fundamental bending mode of a slender beam is *soft* -- its modal stiffness is
several orders of magnitude below ‖K‖ -- so float32 assembly round-off shifts ω₁ by a few percent
(the ring comes out at the wrong speed while energy is still conserved). We enable float64 up front;
``jno.fem`` also warns when a second-order-time form is assembled without it.
"""

import jax

jax.config.update("jax_enable_x64", True)  # soft bending mode: not resolvable in float32
...
# --- figure: the tip ringing vs the analytic modal cosine over two periods ---
...
ax.plot(ts, tip_y, lw=2.2, label="jNO tip (elastodynamics)")
ax.plot(ts, analytic, "k--", lw=1.2, label=r"analytic $\varphi_1\cos(\omega_1 t)$")
ax.set_xlabel("time")
ax.set_ylabel("tip deflection $u_y$")
ax.set_title(f"Ringing cantilever — fundamental mode (rel $L^2$ = {rel:.3f})")
ax.legend(loc="upper right", frameon=False)
ax.margins(x=0)
assets = Path(__file__).resolve().parents[2] / "assets"
assets.mkdir(exist_ok=True)
fig.savefig(assets / "elastodynamics_cantilever_2d.png")
print(f"  saved figure -> {assets / 'elastodynamics_cantilever_2d.png'}")
