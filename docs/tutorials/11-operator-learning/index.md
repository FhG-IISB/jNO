...
| [U-Net 2D — supervised Poisson](unet-poisson-2d.md) | `foundax.unet2d` | Data-driven | Convolutional encoder-decoder with skip connections; multiscale by construction |
| [FieldView — physics-informed FNO](field-view-physics.md) | `foundax.fno2d` | Data + FD-physics | FD PDE residual on a live FNO's grid output, added to the loss as a gradient-carrying physics term |
| [FieldView — wave-equation PINN audit](field-view-wave-equation.md) | `foundax.deeponet` | PINN + FD audit | Train a wave-equation PINN, then verify the **trained network's** grid prediction with second-order `u.tt` finite differences |

All three plug into the same `jno.nn.wrap(...)` interface, so the rest of your training pipeline (callbacks, schedules, checkpointing, evaluation) is identical. Foundation models (PROSE, Poseidon, PDEformer-2) plug in the same way but are out of scope here — see the [Foundation Models](../../foundation_models/index.md) page for the fine-tuning workflow.
