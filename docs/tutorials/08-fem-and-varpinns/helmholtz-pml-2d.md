...
## Complex weak form via `jno.np.i`

`jno.np.i` is the imaginary unit. When a weak form contains it, `jno.fem` solves the problem with a
**real-equivalent** method (it splits each term into real $\mathrm{Re}$/$\mathrm{Im}$ sub-forms,
...
- A complex coefficient (`jno.np.i`) is the only signal needed — `fem.is_complex` is `True` and
