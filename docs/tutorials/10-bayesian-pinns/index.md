...
| 14 | [`pattern_b_bnn_head`](./pattern-b-bnn-head.md) | **Bayesian Last Layer (Pattern B).**  Same MLP as T10, but the body is Adam-trained while the head is SGLD-sampled — both on the **same** pytree.  Phase 15 lifts the v1 block via a `_MixedState` wrapper.  Posterior mean tightens 6× vs T10's random-feature body; ~50 % vs T07's full-net SGLD with comparable bands.  K=1 and K>1 both supported (SAEM-simplified for K>1). | Snoek et al. 2015 §3; Daxberger et al. 2021 §3 |
