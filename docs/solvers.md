...
!!! warning "jNO warns you when this applies"
    `jno.fem` detects a saddle system structurally at build time —
...
Declarative specs — materialized against the assembled operator at solve time. A preconditioner never
changes the converged solution, only the speed, so specs need no gradient path.

| Spec | What it is | When |
|---|---|---|
| `jno.precond.jacobi()` | diagonal | the default — cheapest thing that helps |
| `jno.precond.chebyshev(degree=…)` | fixed-degree polynomial — matvecs and AXPYs only | GPU, where reductions cost; a fixed linear map, so it legally preconditions `cg`/`minres` |
| `jno.precond.nystrom(rank=…)` | randomized low-rank deflation | **SPD only** — a spectrum with a few large outlying eigenvalues, where `jacobi` can be worse than nothing |
| `jno.precond.inner(solver)` | any `jno.solve` solver as the `M⁻¹` application | an inexact block/system solve — iterative inner ⇒ outer `fgmres` |
...
??? note "`jno.precond.nystrom(rank=…)`"
    **randomized Nyström** low-rank preconditioner (Frangella, Tropp &
...
??? note "`jno.precond.saddle(mass_weight=…, laplace_weight=…)`"
    that standard shape as **one call**, for the common case.
...
??? note "`jno.precond.amg(cycles=…)`"
    **hybrid algebraic multigrid**: setup once on the host via the *optional*
...
??? note "`jno.precond.jaxamg(symmetric=…)`"
    GPU AMG via NVIDIA **AmgX** as the `M⁻¹` application (setup and
...
!!! measured "3-D Yeoh phase-field march — 576 DOFs, 8 load steps, CPU"
...
`line_search=` globalizes each sub-solve's Newton steps, and is on by default:
...
#### Over-relaxation
 Alternate minimization *is* a nonlinear block
...
!!! measured "A constant step operator factorizes once, not once per step"
    The step matrix is
...
!!! note "Adjoint memory — the march is gradient-checkpointed"
    Reverse-mode through the default integrator
...
??? note "Bloch (quasi-periodic) ties"
