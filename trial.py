...
net = jno.nn.wrap(fx)
net.optimizer(
    optax.adam(learning_rate=optax.schedules.cosine_decay_schedule(init_value=1e-3, decay_steps=20_000, alpha=1e-5))
)
...
crux.solve(epochs=2_000, batchsize=4, callbacks=[cb]).plot(f"{dir}/training.png")
...
tst_dom.variable(
    "k",
    jax.random.uniform(jax.random.PRNGKey(0), shape=(16, 1, 1), minval=0.1, maxval=1.9),
)
